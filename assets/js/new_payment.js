  ...
  if (customerid) {
    $.ajax({
      ...
      .done(function (data) {
        ...
        $.each(rowlist, function (index, val) {
          $.each(invoicelist, function (index, value) {
            $("#id_invoice_no" + val).append('<option value="' + value + '">' + value + '</option>');
          });
        });
        $(".add_row").parent().show();
      })
