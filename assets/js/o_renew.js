...
function treehouse() {
  ...
  $.each(tree["otl"], function (i, ot) {
    $.each(tree[ot]["oil"], function (j, oi) {
      ...
      if (ot < 4) {
        $("#order_items").append(
          '<tr data-widget="expandable-table" aria-expanded="false" id="parent_' + ot + "_" + oi + '"><td><i class="fas fa-caret-right fa-fw"></i>' + tree[ot][oi]["itm"] + "</td><td>" + tree[ot][oi]["dsp"] + "</td><td>" + getot(ot) + "</td><td>" + tree[ot][oi]["qty"] + "</td><td>" + tree[ot][oi]["uom"] + "</td><td>" + tree[ot][oi]["utp"] + "</td><td>" + tree[ot][oi]["stl"] + '</td><td id="id_buttontool' + ot + "_" + oi + '"></div></td></tr>');
        if (tree[ot][oi]["old"] == false) {
          $("#id_buttontool" + ot + "_" + oi).append('<div class="card-tools"><button type="button" class="btn btn-sm btn-default myorder mr-1" data-oti="' + ot + '" data-oii="' + oi + '"><i class="fas fa-pen text-primary"></i></button>');
        }
      } else {
        $("#order_items").append(
          '<tr id="parent_' + ot + "_" + oi + '"><td>' + tree[ot][oi]["itm"] + "</td><td>" + tree[ot][oi]["dsp"] + "</td><td>" + getot(ot) + "</td><td>" + tree[ot][oi]["qty"] + "</td><td>" + tree[ot][oi]["uom"] + "</td><td>" + tree[ot][oi]["utp"] + "</td><td>" + tree[ot][oi]["stl"] + '</td><td id="id_buttontool' + ot + "_" + oi + '"></td></tr>'
        );
        if (tree[ot][oi]["old"] == false) {
          $("#id_buttontool" + ot + "_" + oi).append('<div class="card-tools"><button type="button" class="btn btn-sm btn-default myorder mr-1" data-oti="' + ot + '" data-oii="' + oi + '"><i class="fas fa-pen text-primary"></i></button></div>');
        }
      }
      if (ot < 4) {
...
function update_tax(s = sgst, c = cgst, i = igst) {
  ...
  $("#add_order_item_cgstcut_txt").text(c);
  $("#add_order_item_igstcut_txt").text(i);

  if (currencyCode === 'INR') {
    if (l_gst.state === "same") {
      // Same state – show SGST + CGST
      $("#col_sub_total").removeClass("col-4").addClass("col-3");
      $("#col_total").removeClass("col-4").addClass("col-3");
      $("#col_sgst").show();
      $("#col_cgst").show();
      $("#col_igst").hide();
    } else {
      // Different state – show IGST
      $("#col_sub_total").removeClass("col-3").addClass("col-4");
      $("#col_total").removeClass("col-3").addClass("col-4");
      $("#col_sgst").hide();
      $("#col_cgst").hide();
      $("#col_igst").show();
    }
  } else {
    // Foreign currency – hide all taxes
    $("#col_sub_total").removeClass("col-3").addClass("col-4");
    $("#col_total").removeClass("col-3").addClass("col-4");
    $("#col_sgst").hide();
    $("#col_cgst").hide();
...
function getgst(customerId) {
  sgst = cgst = igst = 0
  $.ajax({ type: "POST", url: baseUrl + "invoices/gettaxesrate/" + customerId, dataType: "json", encode: true })
    .done(function (resp) {
      $("#itemcard").show()
      l_gst = resp
...
customerId = $("#group_id").data("customer-id");
currencyCode = $("#group_id").data("currency-code");
getgst(customerId);
...
function ordertype_reset() {
  ...
  $("#add_order_sgst_val").text(currency_symbol + 0.00);
  $("#add_order_cgst_val").text(currency_symbol + 0.00);
  $("#add_order_igst_val").text(currency_symbol + 0.00);
  $("#add_order_total_val").text(currency_symbol + 0.00);

  if (currencyCode === 'INR') {
    if (l_gst && l_gst.state === "same") {
      $("#col_sgst").show();
      $("#col_cgst").show();
      $("#col_igst").hide();
      $("#col_sub_total").removeClass("col-4").addClass("col-3");
...
function treeleaves() {
  ...
  if (oti < 4 || oti == 7) {
    ...
    if (oti == 1 || oti == 3 || oti == 7) {
      ...
      tree[oti][item_id]["from"] = tree[oti][item_id]["till"] = "";
    }
...
function order_calculator(id) {
  ...
  $("#add_order_sgst_val").text(currency_symbol + nz(a.toFixed(2)));
  $("#add_order_cgst_val").text(currency_symbol + nz(b.toFixed(2)));
  $("#add_order_igst_val").text(currency_symbol + nz(c.toFixed(2)));
