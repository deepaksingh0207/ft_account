...
class CompanyModel extends Model {
    ...
    public function getList() {
        $sql = "select * from company where 1=1 order by updated_date desc";
        $this->_setSql($sql);
        $user = $this->getAll();
        if (empty($user)){
            return false;
    ...
    public function getNameList() {
        $sql = "select id, name from company where 1=1 and status=1";
        $this->_setSql($sql);
        $user = $this->getAll();
        if (empty($user)){
            return false;
    ...
    public function save($data) {
        ...
        $sql = "INSERT INTO company (" . implode(",", $datafields ) . ") VALUES " .
