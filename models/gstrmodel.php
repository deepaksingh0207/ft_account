...
class GstrModel extends Model {
    ...
    public function getGstrReportList($id) {
        $sql = "SELECT i.id, c.gstin, c.name receiver, i.invoice_no, LEFT(i.invoice_date , 10) invoice_date, i.sub_total invoice_value, CONCAT(LEFT(c.gstin , 2),'-',s.name) pos, 'N' rc, NULL tr, 'Regular B2B' it, NULL egst, 18 rate, invoice_total,NULL cs FROM invoices i inner join customers c on i.customer_id = c.id inner join states s on s.id = c.state";
